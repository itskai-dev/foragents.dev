  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const maybe = data as { error?: string; message?: string; agentHandle?: string; isPremium?: boolean };

        if (!res.ok) {
          throw new Error(maybe?.error || maybe?.message || 'Activation failed');
        }

        if (!cancelled) {
          setState({
            status: 'success',
            agentHandle: maybe.agentHandle || null,
            isPremium: !!maybe.isPremium,
          });
        }
      } catch (err: unknown) {
        if (!cancelled) {
